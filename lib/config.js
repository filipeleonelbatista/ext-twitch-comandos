...
/** URL for viewer OAuth: same origin, else {apiBase}; path /api/auth?type=viewer&channel_id=...&state=viewer:... */
export function getViewerAuthUrl(channelId) {
  const base = getApiBaseUrl();
  const origin = typeof window !== 'undefined' ? window.location.origin : base || '';
  const baseUrl = base ? `${base}` : origin;
  const state = `viewer:${channelId}`;
  return `${baseUrl}/api/auth?type=viewer&channel_id=${encodeURIComponent(channelId)}&state=${encodeURIComponent(state)}`;
}
